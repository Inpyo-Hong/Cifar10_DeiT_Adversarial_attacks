...
criterion = Soft_Distillation_Loss(lambda_balancing=0.5) #논문에서 가장 좋은 Alpha값 0.5
                                                         #Soft Distillation loss 참고하기
...
temperature = 10 #Soft Distillation Temperature
...
for epoch in range(1, NUM_EPOCHES + 1):
    ...
    for batch_idx, (img, labels) in enumerate(trainloader):
        ...
        loss = criterion(teacher_preds, preds, labels, temperature)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        ...
        accuracy = total_correct * 100 / total
        if h_acc <= accuracy:
            save_checkpoint(model, f'Pretrained_best_DeiT(Soft)_T10.pth')
            h_acc = accuracy
